...
def plot_tvalue_vs_complexity(ax, path, field='total_time'):
    f = open(path)
    data = json.load(f)
    no_of_tests = int(data['no_of_tests'])
...
ax.set_title(f"T value vs Ratio of actual time and time complexity O(km) for {n_value} node graphs")
